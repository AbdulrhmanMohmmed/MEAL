...
from app.models import (
    ...
    DataForm, FormSubmission, Distribution,
    DataQualityAssessment, User
)
...
def analytics_overview(
    ...
    total_forms = db.query(DataForm).count()
    ...
    for m in measurements:
        ...
        if ind_id not in indicator_trends:
            ...
            indicator_trends[ind_id] = {
                ...
                "target": indicator.target if indicator else 0,
                "data_points": [],
            }
...
def sample_calculator(
    ...
    if e == 0 or population <= 0:
        raise HTTPException(status_code=400, detail="حجم السكان يجب أن يكون أكبر من صفر")
    ...
    return {
        ...
        "formula": "n = (Z²×p×(1-p)) / e² adjusted for finite population",
    }
...
def run_data_quality_assessment(
    ...
    if total == 0:
        dqa = DataQualityAssessment(
            ...
            total_records=0, complete_records=0,
            accuracy_score=0, timeliness_score=0,
            consistency_score=0, overall_score=0,
            status="critical",
    ...
    for sub in submissions:
        try:
            sub_data = sub.data if isinstance(sub.data, dict) else json.loads(sub.data)
    ...
    dqa = DataQualityAssessment(
        project_id=project_id, form_id=form_id,
        total_records=total, complete_records=complete,
...
def check_duplicates(
    ...
    for d in duplicates:
        matching = db.query(Beneficiary).filter(
            Beneficiary.full_name == d[0],
            Beneficiary.governorate == d[1],
        ).all()
        results.append({
            "name": d[0],
            "governorate": d[1] or "غير محدد",
            "count": d[2],
...
def cross_project_comparison(
    ...
    for p in projects:
        ...
        submissions = db.query(FormSubmission).join(DataForm).filter(
            DataForm.project_id == p.id
        ).count()
        achieved = sum(1 for i in indicators if i.current_value >= i.target)
