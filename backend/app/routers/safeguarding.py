...
def create_report(
    ...
    for attempt in range(5):
        report = SafeguardingReport(
            ...
            **{k: v for k, v in data.items() if hasattr(SafeguardingReport, k) and k != "reference_number"},
        ...
        try:
            ...
            db.rollback()
    raise HTTPException(status_code=400, detail="فشل في إنشاء البلاغ")
...
def create_chs(
    ...
    assessment = CHSAssessment(
        **{k: v for k, v in data.items() if hasattr(CHSAssessment, k)},
